...
def save_unique_entities(data, column_name):
    ...
    data[column_name] = data[column_name].astype(str).apply(preprocess_and_tokenize).apply(lambda token_list: ' '.join(token_list))
...
def visualize(data, tfidf_reduced, method, n_components):
    ...
    if n_components == 3:
        ...
        for cluster_id, color in zip(unique_clusters, colors):
            ...
            ax.scatter(cluster_data[:, 0], cluster_data[:, 1], cluster_data[:, 2], c=color, label=f'Cluster {cluster_id}')
